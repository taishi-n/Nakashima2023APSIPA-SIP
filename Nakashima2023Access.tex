...
\begin{itemize}
  ...
  \item 比較手法
    \begin{itemize}
      % \item マイク回転前後で2個のブロックごと独立にバッチ
      \item \Cref{alg:naive}: 通常のOnline AuxIVA
      \item \Cref{alg:reset}: 回転後に$\Demix _{\ft}, \cov _{\src,\ft}$を初期化
      \item \Cref{alg:sfi}: 回転後に音場補間 $\Obs _{\ft} = \rotMat _{\rotDeg} ^{-1} \rotObs _{\ft}$
      \item \Cref{alg:coord}: 回転した瞬間に座標変換\\
