...
\todo{%
ここで，一般性を失わず，リファレンスマイクを1番目としている．
このとき，$s_1, \dots, s_K$はリファレンスマイク位置でのsource imageを表すことになる．
ステアリングベクトルが時変になるのはCMAが回転しうるためである．
リファレンスマイクも移動するが，その場合でもステアリングベクトルはtime-varyingであり，その状況での各時刻のsource imageを求めたい．
}
...
\end{equation}
ただし，この処理は観測信号のマイク位置を変えるため，本論文の目的である，回転した後の位置におけるsource imageが得られない．
そのため，観測信号ではなく，分離信号を得るためのパラメータに何らかの変換を適用することで，マイクの位置を変えずにマイク回転に頑健なonline BSSを提案する．
...
前述したように，CMAの回転角度と回転時刻が得られていれば，音場補間によってCMAの回転をキャンセルできる．
ただし，音場補間した観測信号は物理的な参照マイクとsource imageとの位置関係を変えるため，実応用に問題が生じる場合がある．
...
\todo{%
  \begin{itemize}
    \item $t$までが回転前, $t+1$で回転後として，回転前までで推定された共分散を回転後の共分散に変換したい
    \item 各時刻フレームの信号を確率過程とみなし，時刻のsumではなく，理想的な$\cov _{\src,\ft}$を$\weight (\var _{\src,\tframe}) \Obs _{\ft} \Obs _{\ft}^{\hermite}$の期待値演算で書き，
      その推定としてオンライン型の更新式を使うという立場で書く
  \end{itemize}
}
\todo{（行き詰まり中…）}
\begin{align}
...
次の4つの手法を比較した．
\begin{description}
  \item[\NaiveIVA] \Cref{subsec:oiva}で説明したOnline AuxIVAをベースラインとして用いる．\Cref{alg:naive}に処理の流れを示す．
  \item[\ResetIVA] CMAの回転後に，分離行列および共分散行列を初期化する．\Cref{alg:reset}に処理の流れを示す．
  \item[\RTObs] CMAの回転後に，フレームごとに観測信号を対して音場補間を適用したあとに分離行列を推定する．\Cref{alg:rtobs}に処理の流れを示す．
    $\Obs _{\ft} = \rotMat ^{\hermite} (\rotSpat _{\tframe}) \rot{\Obs} _{\ft}$
  \item[\RTCov] CMAの回転後に，分離行列および共分散行列を``座標変換''したあとに分離行列を推定する．\Cref{alg:rtcov}に処理の流れを示す．
...
\Cref{fig:plots:sisdr}に\SI{1}{\second}ごとのSI-SDRiを示す．
