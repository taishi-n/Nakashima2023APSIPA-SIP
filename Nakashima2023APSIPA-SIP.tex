...
Assuming that the microphone array rotates only once at a specific time frame $\tframe$, we analyze how the proposed method, as given in \cref{eq:cov:rot}, differs from the original formula in \cref{eq:cov}.
In \cref{eq:cov}, the value of $\cov _{\src,\ft}$ is updated online.
However, since the angles of the microphone array differ between time frames $\tframe - 1$ and $\tframe$, the amalgamation of $\cov _{\src,\ft}$, which holds information before the rotation, and $\Obs _{\ft}$, which holds information after the rotation, leads to an inaccurate estimation of the demixing matrix $\Demix _{\ft}$.
If the microphone array does not rotate after time frame $\tframe$, the observation information before the rotation is gradually diminished, and $\cov _{\src,\ft}$ and $\Demix _{\ft}$ are expected to slowly converge to their values at the post-rotation position of the microphone array.
However, this convergence is expected to be a time-consuming process.
Conversely, in \cref{eq:cov:rot}, all observed signals are translated into observations at the reference position.
Thus, even if the microphone array rotates at time $\tframe$, there is no substantial discrepancy between $\cov _{\src,\ft}$ and $\Obs _{\ft}$, and the influence of rotation on the estimation of the demixing matrix $\Demix _{\ft}$ is expected to be reduced.

%A similar scheme which is for beamforming has been proposed in \cite{Wakabayashi:2023:ASLP}.
Note that \cref{alg:sfiva-o} virtually changes the microphone position of the observed signal.
...
\begin{align}
  \Demix _{\freq,\rotFra - 1} &\gets \refpos{\Demix} _{\freq,\rotFra - 1} \rotMat ^{\hermite} (\rotSpat), \label{eq:sfiiva:demix} \\
  \cov _{\src,\freq,\rotFra - 1} &\gets \rotMat (\rotSpat) \refpos{\cov} _{\src,\freq,\rotFra - 1} \rotMat ^{\hermite} (\rotSpat). \label{eq:sfiiva:cov}
\end{align}
  ...
  \begin{pseudo}
    ...
    for $\tframe = 1,\, \dots,\, \Tframe$ \\+
      ...
      for $\freq = 1, \dots, \Freq$ \\+
        ...
        for $\src = 1,\, \dots,\, \Src$ \\+
          {$\cov _{\src,\ft[-1]}$} $\gets$ $\rotMat (\varDelta\rotSpat _{\tframe}) \cov _{\src,\ft[-1]} \rotMat ^{\hermite} (\varDelta \rotSpat _{\tframe})$ \ct{\eqref{eq:sfiiva:cov}}\\--
      for $\itr = 1,\, \dots,\, \Itr$ \\+
        for $\src = 1,\, \dots,\, \Src$ \\+
          {$\var _{\src,\tframe}$} $\gets$ $\sqrt{\sum _{\freq = 1} ^{\Freq} \abs{\demix _{\src,\ft} ^{\hermite} \Obs _{\ft}} ^2}$ \ct{\eqref{eq:var}} \\
          for $\freq = 1, \dots, \Freq$ \\+
...
To simulate the rotation of the CMA instantaneously, the source images were generated when the CMA was rotated \SI{40}{\degree} counter-clockwise from the horizontal axis and was stitched together in the 0--30\si{\second} and 30--60\si{\second} intervals, respectively.
...
\Cref{tab:sdr} shows the averaged over samples and channels before and after CMA rotation with a measuring error.
...
\begin{table}[t]
  \caption{%
    Average SI-SDRi in decibels before and after CMA rotation with measuring error.
  ...
  \begin{minipage}[t]{.45\linewidth}
    \centering
    \subcaption{Measured angle of \SI{40}{\degree}, which is equal to the ground truth.}%
    \label{tab:sdr:98}
    \begin{tabular}{lSS}
      \toprule
  ...
  \begin{minipage}[t]{.45\linewidth}
    \centering
    \subcaption{Measured angle of \SI{60}{\degree}, which is differ from the true angle of \SI{40}{\degree}.}%
    \label{tab:sdr:98}
    \begin{tabular}{lSS}
      \toprule
        Method   & \multicolumn{2}{c}{Time (s)} \\ \cmidrule{2-3}
