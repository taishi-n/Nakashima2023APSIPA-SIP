...
\end{equation}
$\cov _{\src,\freq}$ is the \emph{weighted covariance matrix} of the observed signals defined as:
\begin{align}
  ...
  \var _{\src,\tframe} &= \sqrt{\sum _{\freq=1} ^{\Freq} \lvert \demix ^{\hermite} _{\src,\freq} \Obs ^{\nohermite}_{\ft} \rvert ^2} \label{eq:var:batch},
\end{align}
where
...
\begin{align}
  \demix _{\src,\freq} &\gets (\Demix _{\freq} \cov _{\src,\freq}) ^{-1} \eye _{\src} \label{eq:ip:proj:batch}, \\
  \demix _{\src,\freq} &\gets \frac{{\demix _{\src,\freq}}}{{\sqrt{\demix _{\src,\freq} ^{\hermite} \cov _{\src,\freq} \demix _{\src,\freq}}}} \label{eq:ip:norm:batch},
\end{align}
...
Thanks to this incremental update rule, we can directly IP to estimate time-varying demixing matrices $\{\Demix _{\ft}\} _{\freq}$ at each time frame $\tframe$:
\begin{align}
