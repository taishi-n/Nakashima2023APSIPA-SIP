...
On the other hand, for practical applications, it is necessary to consider time variations of the ATS, such as microphone or source movements.
Many BSS methods have been proposed based on block batch \cite{Koldovsky:2019:ICASSP,Koldovsky:2021:SP,Jansky:2022:ASMP} or online processing \cite{Kim:2010:CASI,Taniguchi:2014:HSCMA}.
...
In recent years, several challenges have been held on the subject of speech processing under dynamic environments.
The Clarity Challenge is a challenge to improve speech intelligibility in hearing aids and includes data on the listeners' head movements \cite{Akeroyd:2023:ICASSP}.
The SPEAR challenge is a speech enhancement challenge for head-worn hearable devices, and extensive datasets have been distributed that include speaker and head movement data \cite{Donley:2021:arxiv,Guiraud:2022:IWAENC}.
Thus, there is an increasing demand for speech enhancement or source separation methods that can perform robustly in dynamic environments.
...
% しかし，この処理は仮想的に観測信号のマイク位置を変えるため，回転後の位置におけるsource imageが得られず，実応用に問題が生じる場合がある．
