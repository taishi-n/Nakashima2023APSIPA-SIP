...
We consider online BSS problem in a situation where the CMA is rotated by an angle $\rotSpat _{\tframe}$.
...
\end{itemize}
Note that $\rotFra$ and $\rotSpat$ are assumed to be known or estimated in this paper.
...
\begin{equation}
  \rotMat _{\rotSpat} =
  \begin{bmatrix}
    \rotmat _{0, 0}(\rotSpat) & \rotmat _{0, 1}(\rotSpat) & \dots & \rotmat _{0, \Mic - 1}(\rotSpat) \\
    \rotmat _{1, 0}(\rotSpat) & \rotmat _{1, 1}(\rotSpat) & \dots & \rotmat _{1, \Mic - 1}(\rotSpat) \\
    \vdots & \vdots  & \ddots & \vdots \\
    \rotmat _{\Mic - 1, 0}(\rotSpat) & \rotmat _{\Mic - 1, 1}(\rotSpat) & \dots & \rotmat _{\Mic - 1, \Mic - 1}(\rotSpat)
  \end{bmatrix}
  \in \C ^{\Mic\times\Mic}
\end{equation}
is called \emph{rotation matrix}, and its $(k,l)$element is calculated as:
\begin{align}
  \rotmat _{k,l}(\rotSpat) &=
    \begin{cases}
...
and $\rotIdx = l - k - \rotSmpl\; (k, l = 0, \dots, \Mic - 1)$ \cite{Wakabayashi:2023:ASLP}.
This equation is an alternative expression for (3) in \cite{Wakabayashi:2023:ASLP}.
...
\begin{itemize}
  \item 比較手法
    ...
    % \begin{itemize}
    %   \item $\cov _{\src,\ft[-1]}$はそれ以前のすべての観測の情報を含んでいるので，それだけに$\rotMat _{\rotSpat}$をかけて，あとは観測をそのまま使って更新すればいいはず
    % \end{itemize}
    \end{itemize}
  \item パラメータ
    \begin{itemize}
...
\begin{algorithm}{Online AuxIVA with sound field interpolation for observed signal (RT-obs)}{sfi}
  \begin{pseudo}
    for $\tframe = 1,\, \dots,\, \rotFra - 1$ \\+
      $\{\Demix _{\ft}\}_{\freq},\, \{\cov _{\src,\ft}\} _{\src,\freq} \gets$ \pr{OIVA}(\{\Obs _{\ft}\} _{\freq}, \{\Demix _{\ft[-1]}\}_{\freq},\, \{\cov _{\src,\ft[-1]}\} _{\src,\freq}) \\-
    $\rotMat _{\rotSpat}$ $\gets$ \pr{CalcRotationMatrix}(\Mic, \rotSpat) \\
    for $\tframe = \rotFra,\, \dots,\, \Tframe$ \\+
