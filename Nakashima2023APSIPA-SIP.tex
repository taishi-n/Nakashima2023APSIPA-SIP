...
In the self-rotation-robust beamforming proposed in \cite{Wakabayashi:2023:ASLP},
the signals observed by the CMA with angle $\theta_t$ are transformed frame by frame to what would have been observed at the reference position, namely at angle $\theta=0$, using the sound field interpolation, and then beamforming is applied to the transformed signals. In this subsection, we apply a similar approach to OIVA.
%to propose a robust online BSS method against the rotation of CMA.
...
%  \Obs _{\ft} &\gets %\left(\rotMat ^{\hermite} 
%(\rotSpat _{\tframe}) \Obs %_{\ft}\right) \coloneqq %\refpos{\Obs} _{\ft}.
%\end{align}
...
\Cref{alg:sfiva-o} summarises OIVA using this approach.
In this method, the transformation using SFT:
\begin{align}
  \refpos{\Obs} _{\ft}
   &\gets \rotMat ^{\hermite} (\rotSpat _{\tframe}) \Obs _{\ft}.
\end{align}
is simply applied as a pre-processing for OIVA
and the online update of the weighted covariance matrix $V_{k,f,t}$ is performed by the transformed signal $\refpos{\Obs} _{\ft}$.
