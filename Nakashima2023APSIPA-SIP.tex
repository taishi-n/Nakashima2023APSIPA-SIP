...
\begin{abstract}
  ...
  In this paper, we exploit a sound field interpolation (SFI) for circular microphone arrays (CMAs) with OIVA.
...
However, in practical applications it is necessary to consider time variations of the ATS, such as microphone movements.
...
Multi-channel acoustic signal processing techniques that consider the dynamic environment have attracted much attention in recent years.
...
Furthermore, competitions have been organised and datasets developed on the subject of speech processing in dynamic scenarios.
...
OIVA estimates demixing matrices frame-by-frame manner and can track smooth environmental changes in ATS, such as slow movement of sources.
...
As described in \cref{sec:proposed}, naive combination of SFI and OIVA has a problem for practical applications.
...
The online updates of $\cov _{\src,\ft}$ \eqref{eq:cov} enable the OIVA to progressively track the slow movement of sound sources or microphones.
...
\Cref{alg:sfiva-o} summarises OIVA using SFI.
...
\begin{figure}[t]
  ...
  \begin{minipage}[t]{.32\linewidth}
    ...
    \subcaption{Sources are randomly placed in orange shaded area.}%
...
\begin{table}[t]
  \caption{Average SI-SDRi before and after CMA rotation with estimation error. True angle was \SI{40}{\degree}. Length of simulated speech signals is \SI{60}{\second} long, and CMA was instananeously rotated at \SI{30}{\second}. Forgetting factor $\forget = 0.98$.}%
  \label{tab:sdr}
  \centering
  \sisetup{%
    table-format=2.2,
  ...
  \begin{minipage}[t]{.45\linewidth}
    ...
    \begin{tabular}{lSS}
      \toprule
        Method   & \multicolumn{2}{c}{CMA rotation} \\ \cmidrule{2-3}
                 &       {Before} &       {After} \\
      \midrule
  ...
  \begin{minipage}[t]{.45\linewidth}
    \centering
    \subcaption{Estimated angle of \SI{60}{\degree}, which is differ from the true angle of \SI{40}{\degree}.}%
    \label{tab:sdr:98}
    \begin{tabular}{lSS}
      \toprule
        Method   & \multicolumn{2}{c}{CMA rotation} \\ \cmidrule{2-3}
