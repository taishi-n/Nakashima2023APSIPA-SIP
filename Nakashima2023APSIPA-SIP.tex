...
\begin{algorithm}{SFI-based Online AuxIVA using Transformation of Latest Observed Signal (\SFIIVAo)}{sfiva-o}
  \textbf{Input:} $\{\Obs _{\ft}\} _{\ft}$, $ \{\Demix _{\freq,\rotFra-1}\}_{\freq}$, $\{\cov _{\src,\freq,\rotFra-1}\}_{\src,\freq}$, $\forget$, $\rotMat(\rotSpat)$ \\
  \textbf{Output:} $\{\Est _{\ft}\} _{\ft}$
  \begin{pseudo}
...
\todo{%
角度の測定誤差があるとき，4.1 の手法は誤差が蓄積する，4.2 の方は忘却係数で忘れていくので，間違っていてもその影響は消えていくはず？
}
...
\begin{algorithm}{SFI-based Online AuxIVA with Transformation of Pre-update Demixing and Covariance Matrices (\SFIIVAm)}{sfiva-m}
  \textbf{Input:} $\{\Obs _{\ft}\} _{\ft}$, $ \{\refpos{\Demix} _{\freq,\rotFra-1}\}_{\freq}$, $\{\refpos{\cov} _{\src,\freq,\rotFra-1}\}_{\src,\freq}$, $\forget$, $\rotMat(\rotSpat)$ \\
  \textbf{Output:} $\{\Est _{\ft}\} _{\ft}$
  \begin{pseudo}
...
The reference signal for the SI-SDR evaluation was the source image recorded with a rotated CMA.
\begin{figure}[t]
  \centering
...
Notably, \ResetIVA{} has the lowest performance before CMA rotation among all forgetting factors, which gets worse as the forgetting factor approaches 1.
In contrast, the proposed \SFIIVAm{} and \SFIIVAo{} methods maintained performance even after the CMA rotation.
...
\Cref{fig:plots:bp} shows the beam patterns of demixing matrices calculated by each method.
...
As shown in \cref{fig:plots:bp}, the results of \cref{fig:plot:bp:ref} and \cref{fig:plot:bp:sfiivao} were similar to each other, and only \cref{fig:plot:bp:sfiivam} was different from others.
...
\subsection*{\todo{Results for Large Dataset (WORK IN PROGRESS)}}
\todo{TODO: explanations for \cref{fig:plot:hist}, \cref{fig:plot:box}, and \cref{tab:sdr}.}
\begin{figure}[t]
  \begin{minipage}[t]{\textwidth}
    \centering\includegraphics{figures/plots/online/hist_900.pdf}\subcaption{$\forget = 0.90 $}\label{fig:plot:hist:900}
  \end{minipage}
  \begin{minipage}[t]{\textwidth}
    \centering\includegraphics{figures/plots/online/hist_950.pdf}\subcaption{$\forget = 0.95 $}\label{fig:plot:hist:950}
  \end{minipage}
  \begin{minipage}[t]{\textwidth}
    \centering\includegraphics{figures/plots/online/hist_980.pdf}\subcaption{$\forget = 0.98 $}\label{fig:plot:hist:980}
  \end{minipage}
  \begin{minipage}[t]{\textwidth}
    \centering\includegraphics{figures/plots/online/hist_990.pdf}\subcaption{$\forget = 0.99 $}\label{fig:plot:hist:990}
  \end{minipage}
  \caption{Histogram of segmental SI-SDR improvements (SI-SDRi) before and after CMA rotation.}%
  \label{fig:plot:hist}
\end{figure}
\begin{figure}[t]
  \begin{minipage}[t]{.5\textwidth}
    \centering\includegraphics{figures/plots/online/box_900.pdf}\subcaption{$\forget = 0.90 $}\label{fig:plot:hist:900}
    \centering\includegraphics{figures/plots/online/box_950.pdf}\subcaption{$\forget = 0.95 $}\label{fig:plot:hist:950}
  \end{minipage}
  \begin{minipage}[t]{.5\textwidth}
    \centering\includegraphics{figures/plots/online/box_980.pdf}\subcaption{$\forget = 0.98 $}\label{fig:plot:hist:980}
    \centering\includegraphics{figures/plots/online/box_990.pdf}\subcaption{$\forget = 0.99 $}\label{fig:plot:hist:990}
  \end{minipage}
  \caption{Box plot of segmental SI-SDR improvements (SI-SDRi) before and after CMA rotation.}%
  \label{fig:plot:box}
\end{figure}
\begin{table}[t]
  \caption{Mean and standard deviation of SI-SDRi before and after CMA rotation.}%
  \label{tab:sdr}
  \centering
  \sisetup{table-format=2.2, table-alignment-mode=format, table-auto-round=true}
  \begin{tabular}{lllSS}
    \toprule
        &    &          & \multicolumn{2}{l}{SI-SDRi (dB)} \\
        &    &          &         {mean} &        {std} \\
      Forgetting factor $\forget$ & Time (s) & Method & & \\
    \midrule
      0.90 & 30 & Naive    &   0.244856 & 5.000747 \\
           &    & Reset    &  -4.830337 & 5.555344 \\
           &    & SFIIVA-M &   5.065312 & 4.200715 \\
           &    & SFIIVA-O &   5.127707 & 4.291668 \\ \cmidrule{2-5}
           & 59 & Naive    &   6.371168 & 3.451660 \\
           &    & Reset    &   6.404149 & 3.460501 \\
           &    & SFIIVA-M &   6.436108 & 3.435811 \\
           &    & SFIIVA-O &   6.143079 & 3.535327 \\ \hline
      0.95 & 30 & Naive    &   2.849818 & 3.579884 \\
           &    & Reset    &  -7.059805 & 7.959535 \\
           &    & SFIIVA-M &  13.705048 & 3.696854 \\
           &    & SFIIVA-O &  13.451307 & 3.680528 \\ \cmidrule{2-5}
           & 59 & Naive    &  16.652346 & 2.755190 \\
           &    & Reset    &  16.704147 & 2.742908 \\
           &    & SFIIVA-M &  16.645762 & 2.753046 \\
           &    & SFIIVA-O &  15.548719 & 2.507915 \\ \hline
      0.98 & 30 & Naive    &   4.000305 & 3.203627 \\
           &    & Reset    & -10.361193 & 8.261436 \\
           &    & SFIIVA-M &  18.796094 & 3.316340 \\
           &    & SFIIVA-O &  17.988841 & 3.516160 \\ \cmidrule{2-5}
           & 59 & Naive    &  25.698304 & 2.652934 \\
           &    & Reset    &  23.402043 & 2.931695 \\
           &    & SFIIVA-M &  25.623199 & 2.592745 \\
           &    & SFIIVA-O &  22.130434 & 3.686233 \\ \hline
      0.99 & 30 & Naive    &   2.062067 & 4.680030 \\
           &    & Reset    & -11.815238 & 8.516685 \\
           &    & SFIIVA-M &  11.242948 & 7.068764 \\
           &    & SFIIVA-O &  10.706279 & 6.975261 \\ \cmidrule{2-5}
           & 59 & Naive    &  23.513201 & 4.669065 \\
           &    & Reset    &  14.089287 & 5.852374 \\
           &    & SFIIVA-M &  24.893788 & 3.912823 \\
           &    & SFIIVA-O &  21.706356 & 4.382981 \\
    \bottomrule
  \end{tabular}
\end{table}
