...
In the self-rotation-robust beamforming proposed in \cite{Wakabayashi:2023:ASLP}, the signals observed by the CMA with angle $\theta_t$ are transformed frame by frame to what would have been observed at the reference position, namely at angle $\theta=0$, using the sound field interpolation, and then beamforming is applied to the transformed signals.
In this subsection, we apply a similar approach to OIVA.
...
\end{align}
is simply applied as a pre-processing for OIVA and the online update of the weighted covariance matrix $\cov _{\src,\ft}$ is performed by the transformed signal $\refpos{\Obs} _{\ft}$.
