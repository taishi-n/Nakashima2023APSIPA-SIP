...
We consider online BSS problem in a situation where the CMA is rotated by an angle $\rotDeg _{\tframe}$.
...
\begin{itemize}
  \item \textcolor{red}{一般性を失わず1個目のマイクをリファレンスとして，sがリファレンス位置でのsource imageとして書く}
  \item \textcolor{red}{リファレンスマイク位置でのsource imageを求める}
  \item \textcolor{red}{BSSの説明よりも前に，CMAが回転することを書きたい}
\end{itemize}
...
We consider a situation where CMA is rotated of degree $\rotDeg$ at time frame $\rotFra$ and let $\irot{\Obs} _{\ft}$ be the observed signal recorded in the position before CMA was rotated (\emph{reference position}).
...
\end{equation}
where $\rotMat _{\rotDeg} \in \C ^{\Mic\times\Mic}$ is called \emph{rotation matrix}.
$\rotMat _{\rotDeg}$ is calculated as:
\begin{align}
  \rotmat _{k,l}(\rotDeg) &=
    \begin{cases}
      \displaystyle\frac{\twid ^{- \Rot (\frac{\Mic}{2} - 1) }}{\Mic} \frac{1 - \twid ^{\Rot \Mic}}{1 - \twid ^{\Rot}} & (\text{$\Mic$ is even}),\\[1em]
      \displaystyle\frac{\twid ^{- \Rot \frac{(\Src - 1)}{2}}}{\Mic} \frac{1 - \twid ^{\Rot \Mic}}{1 - \twid ^{\Rot}} & (\text{$\Mic$ is odd}),
    \end{cases}\label{eq:rot:twid} \\
  &=
    \begin{cases}
      \displaystyle \frac{\sinc{(\Rot)}}{\sinc{\left(\frac{\Rot}{\Mic}\right)}} \twid ^{\frac{\Rot}{2}} & (\text{$\Mic$ is even}),\\[1em]
      \displaystyle \frac{\sinc{(\Rot)}}{\sinc{\left(\frac{\Rot}{\Mic}\right)}} & (\text{$\Mic$ is odd}),
    \end{cases}\label{eq:rot:sinc}
\end{align}
where $\twid \coloneqq \exp (j \frac{2\pi}{\Mic})$ is a twiddle factor in $\Mic$-points FFT, $\rotmat _{k,l} (\rotDeg)$ is the $(k,l)$ element of $\rotMat _{\rotDeg}$, and $\Rot = l - k - \rotDeg$ \cite{Wakabayashi:2023:ASLP}.
...
\begin{itemize}
  \item \todo{音場補間と音源分離を組み合わせただけのものを書く}
  \item \todo{それではだめだという説明を入れる}
    \begin{itemize}
      \item \todo{求めたいのは回転後のリファレンス位置での音像であって，音場補間を行うとマイク位置が変わってしまう}
    \end{itemize}
\end{itemize}
