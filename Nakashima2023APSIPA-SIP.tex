...
Note that  the separated signals obtained by this approach are the source images at the reference microphone of the CMA with the reference angle, regardless of the CMA rotation.
...
In this subsection, we propose another online BSS that is robust against CMA rotation and outputs the source image at the latest microphone position.
...
\begin{description}
  ...
  \item[\SFIIVAo] Apply SFI to signals observed by CMA rotation and transformed frame by frame. (See \Cref{alg:sfiva-o}.)
    % \\ $\Obs _{\ft} = \rotMat ^{\hermite} (\rotSpat _{\tframe}) \rot{\Obs} _{\ft}$
  \item[\SFIIVAm] Apply parameter transformation to the weighted covariance and demixing matrices after CMA rotation.  (See \Cref{alg:sfiva-m}.)
