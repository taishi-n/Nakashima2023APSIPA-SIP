...
\creditline{%
  ...
  This work was supported by JSPS KAKENHI Grant Number 22KJ2548 and JST CREST Grant Number JPMJCR19A3.
}
...
Popular approaches for BSS include independent vector analysis (IVA) \cite{Kim:2006:ASL,Hiroe:2006:ICA}, auxiliary-function-based IVA (AuxIVA) \cite{Ono:2011:WASPAA}, and their extensions \cite{Kitamura:2016:ASLP,Nugraha:2020:SPL,Brendel:2020:SP}.
...
%     \centering\includegraphics{figures/diagrams/sfiiva-o.pdf}\subcaption{SFIIVA-Obs}\label{fig:sfiiva-o}
%   \end{minipage}
%   \begin{minipage}[t]{\linewidth}
%     \centering\includegraphics{figures/diagrams/sfiiva-m.pdf}\subcaption{SFIIVA-Par}\label{fig:sfiiva-m}
%   \end{minipage}
...
\Cref{alg:sfiva-o} summarizes OIVA using this approach, and \cref{fig:sfiiva-o} shows the system diagram.
...
Each source was randomly placed at least \SI{1}{\metre} from the center of the CMA and within an angle ranging from $\frac{360\si{\degree}}{\Src} \src$ to $\frac{360\si{\degree}}{\Src} (\src - 1) + \frac{180\si{\degree}}{\Src}\; (\src = 1, \dots, \Src)$.
\Cref{fig:layout:exp} illustrates the range of sources' placements and its example.
...
\begin{description}
  ...
  \item[\SFIIVAo] Apply SFI to the latest observation and estimate the source image at the fixed position (\Cref{alg:sfiva-o}).
  \item[\SFIIVAm] Apply SFI to the weighted covariance and demixing matrices as parameter transformation and estimate the source image at the position rotated with CMA (\Cref{alg:sfiva-m}).
\end{description}
